...
" 可以控制可见的折叠层次，只显示折叠层次小于等于参数的行的内容，等于0时只显示
" 最高层的行，其他层次的行都会被折叠
set foldlevel=99
" 设置左边的可视折叠线索，并设置它占用的宽度
set foldcolumn=3
...
" HTML 文件的一般配置，将缩进设置为2
autocmd FileType html set tabstop=2 shiftwidth=2 expandtab

" jinja 文件的一般配置，将缩进设置为2
autocmd FileType jinja set tabstop=2 shiftwidth=2 expandtab 
